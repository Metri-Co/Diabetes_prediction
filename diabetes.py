...
##########################################
##########################################
### Feature engineering + white noise ####
##########################################
##########################################
...
plt.savefig('Original dataframe.jpg', dpi = 300)

for i in range(2):
    data = add_gnoise(data,'diabetes', 1)
...
plt.xlabel('label')
plt.title('Original dataframe')
plt.savefig('Augmented dataframe.jpg', dpi = 300)
...
clf = DecisionTreeClassifier(min_samples_leaf=1, max_depth= None, criterion = 'entropy')
...
clf_auc = roc_curve(y_test, clf_proba, clf_y_pred, estimator_name= 'DT classifier')
...
plt.title("Models' performance with white noise")
plt.ylim([0,1.1])
#plt.show()
plt.savefig('Diabetes/Diabetes engineered models.jpg', dpi =300)

targets = ['Non-diabetes', 'diabetes']
